...
weights_path = "models\custom_yolov5s.pt"
# Load YOLOv5 model
...
def detect():
    ...
    try:
        if request.method == "POST":
            ...
            keyframe_interval = (
                fps // 2
            )  # Set the keyframe interval based on frame rate
            ...
            cleaned_frame_timestamp = {
                int(value / fps) for value in frame_timestamp_set
            }
            ...
            df["frame_detected_secs"] = pd.to_timedelta(
                df["frame_detected_secs"], unit="s"
            )
